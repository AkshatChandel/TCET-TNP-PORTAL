...
class TrainingController extends Controller
{
    ...
    public function create()
    {
        ...
        return view("admin.lecture.create", ["branches" => $branches, "AcademicSessions" => $AcademicSessions]);
    }
    ...
    public function edit($TrainingLectureId)
    {
        ...
        return view("admin.lecture.edit", ["lecture" => $lectureDetails, "lectureBranches" => $lectureBranches, "branches" => $branches, "AcademicSessions" => $AcademicSessions]);
    }

    public function editLecture($TrainingLectureId, Request $request)
    {
        $Branches = $request->Branch_Id;

        if ($Branches != null && count($Branches) != 0) {
            $training_lecture = Training_Lecture::find($TrainingLectureId);
            $training_lecture->Lecture_Name = $request->Lecture_Name;
            $training_lecture->Lecture_Description = $request->Lecture_Description;
            $training_lecture->Lecture_Code = $request->Lecture_Code;
            $training_lecture->Lecture_DateTime = $request->Lecture_DateTime;
            ...
            $deletedRows = Training_Lecture_Branch::where('Training_Lecture_Id', $TrainingLectureId)->delete();
    ...
    public function viewLectureDetails($TrainingLectureId)
    {
        ...
        return view("admin.lecture.view", ["lecture" => $lectureDetails, "lectureBranches" => $lectureBranches]);
    }
}
